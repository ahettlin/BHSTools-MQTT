...
def fmt_raw(cmd, arg):
	return arg
	#return arg.decode("utf-8") if arg else ""

def fmt_hexdump(cmd, arg):
	return fmt_raw(cmd, arg)
	#return hexdump(arg)
...
command_info = {
	...
	16:		('Echo Test', [17], fmt_raw, arg_hex_raw),
	...
	1000:	('Arm System', [1100], fmt_raw, arg_hex_raw),
...
class CommandSender(VirtDevice):
	def __init__(self, ibus, cmd, arg, callback):
	...
	def on_ping(self):
		t = time.time()
		delta = t - self.last_tx
		if delta >= 1 and self.complete == False:
			self.last_tx = t
	...
	def handle_cmd_nosync(self, cmd, arg, synced):
		try:
			if cmd in self.response_cmds and self.complete == False:
				if self.response_out is not None:
					#print(file=sys.stderr)
					response = self.response_out(cmd, arg)
	...
	def on_complete(self):
		self.complete = True
		self.ibus.remove_listener(self)



def send_command(ibus, opcode, args, callback) -> CommandSender:	
	args = args or []
	try:
		cmd = int(opcode)
		...
		print("{} is not a recognized command. If it's a valid one, try using testbed.py.".format(opcode), file=sys.stderr)
	...
	try:
		...
		error = 'Error parsing arguments: {}'.format(ex)
		print(error, file=sys.stderr)
		callback(error)
		return

	return CommandSender(ibus, cmd, arg, callback)